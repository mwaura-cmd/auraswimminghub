...
export function ParallaxHero() {
  ...
  return (
    <section className="section-shell relative overflow-hidden rounded-[2rem] border border-cyan-300/20 bg-[#08376b] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.03),0_24px_80px_rgba(1,8,24,0.45)] md:px-6 md:py-6">
      ...
      <div className="relative grid min-h-[74vh] gap-6 md:grid-cols-[1.08fr_0.92fr] md:gap-8 lg:min-h-[78vh]">
        <motion.div
          style={{ y: imageY }}
          initial={{ opacity: 0, x: -18 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="relative overflow-hidden rounded-[1.75rem] border border-white/70 bg-slate-950 shadow-[0_16px_60px_rgba(1,8,24,0.4)]"
        >
          ...
          <div className="absolute inset-0 rounded-[1.75rem] ring-1 ring-white/40 ring-inset" />
        </motion.div>

        <div className="relative flex items-center justify-center px-2 py-6 text-center md:px-4 md:text-left">
          <motion.div
            style={{ y: titleY }}
            ...
            className="max-w-xl"
          >
            <p className="text-sm font-medium uppercase tracking-[0.28em] text-white/90 md:text-base">Aura Swimming Hub</p>
            <h1 className="mt-5 text-5xl leading-[0.92] text-cyan-300 md:text-6xl lg:text-7xl xl:text-[5.35rem]">
              Dive Into
              <span className="block text-cyan-200">Greatness</span>
            </h1>
            <p className="mx-auto mt-6 max-w-lg text-sm leading-7 text-cyan-50/80 md:mx-0 md:text-base">
            ...
            <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row md:items-start">
              <Link href="/book" className="btn-primary inline-flex items-center gap-2 text-sm">
                Book a Session <ArrowRight size={16} />
              </Link>
              <Link href="/programs" className="rounded-full border border-cyan-200/60 bg-white/5 px-5 py-3 text-sm font-semibold text-cyan-50 transition hover:border-white/90 hover:bg-white/10">
                Explore Programs
              </Link>
            </div>
          </motion.div>
